...
if response.ok:
    ...
    print(
        f"Interface: {response_json['Cisco-IOS-XE-interfaces-oper:interface']['name']}"
    )
    print(
        f"Description: {response_json['Cisco-IOS-XE-interfaces-oper:interface']['description']}"
    )
    print(
        f"IPv4 Address: {response_json['Cisco-IOS-XE-interfaces-oper:interface']['ipv4']}"
    )
    if (
        response_json["Cisco-IOS-XE-interfaces-oper:interface"]["admin-status"]
        == "if-state-up"
    ):
